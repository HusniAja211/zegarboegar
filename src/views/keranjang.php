...
// Karena kasir, total = subtotal (tanpa ongkir)
                ...
                <?php else: ?>
                    <?php foreach ($keranjang as $id => $item): ?>
                        <div class="flex flex-col sm:flex-row items-start sm:items-center bg-white p-4 rounded-xl shadow-lg border border-blue-100 transition duration-300 hover:shadow-xl"
                             data-cart-id="<?= htmlspecialchars($id) ?>">
                            
                            <div class="flex-grow w-full mt-3 sm:mt-0">
                                <h2 class="text-lg font-semibold text-blue-800">
                                    <?= htmlspecialchars($item['nama_produk'] ?? 'Produk') ?>
                                </h2>
                                <p class="text-sm text-gray-500">
                                    Harga Satuan: <?= formatRupiah($item['harga'] ?? 0) ?>
                                </p>
                                ...
                                <div class="flex items-center space-x-4 mt-2">
                                    <div class="flex items-center border border-blue-300 rounded-md">
                                        <button
                                            type="button"
                                           data-id="<?= $item['id_produk'] ?>"data-action="minus" 
                                            class="px-3 py-1 text-blue-700 hover:bg-blue-50 transition duration-150">−</button>

                                        <input type="text" value="<?= (int)($item['qty'] ?? 0) ?>" 
                                               class="w-10 text-center border-x border-blue-300 focus:outline-none text-blue-900 bg-white qty-display" readonly>

                                        <button
                                            type="button"
                                              data-id="<?= $item['id_produk'] ?>" data-action="plus"
                                            class="px-3 py-1 text-blue-700 hover:bg-blue-50 transition duration-150">+</button>
                                    </div>

                                    <button
                                        type="button"
                                        data-id="<?= $item['id_produk'] ?>" data-action="delete"
                            ...
                            <div class="mt-4 sm:mt-0 flex-shrink-0 text-right w-full sm:w-auto">
                                <p class="text-xl font-bold text-blue-900 line-total">
                                    <?= formatRupiah( ($item['harga'] ?? 0) * ($item['qty'] ?? 0) ) ?>
                                </p>
                            </div>
                        </div>
                    <?php endforeach; ?>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white p-6 rounded-xl shadow-lg border-t-4 border-blue-600 sticky top-8" id="ringkasan-pembayaran">
                    ...
                    <div class="mb-4 bg-blue-50 p-3 rounded-lg border border-blue-200">
                        ...
                        <p class="text-lg font-bold text-blue-900" id="poin-saat-ini">
                    ...
                    <div class="space-y-3 text-gray-700">
                        ...
                        <div class="pt-4 border-t border-blue-200 flex justify-between text-xl font-extrabold text-blue-900">
                            ...
                            <span id="total-tagihan" data-total-kotor="<?= $totalTagihan ?>">
                    ...
                    <button id="btn-checkout" type="button" class="mt-6 w-full py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition duration-300 transform hover:scale-[1.01]" disabled>
